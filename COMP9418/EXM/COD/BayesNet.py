...
class BayesNet():
    ...
    def bucketVE(self, order):
        buck = {i:[] for i in order}
        visited = {i:False for i in order}
        ...
        for var in order:
            ...
            if var == list(buck.keys())[-1]:
                return newFactor

            for nex in order:
                if visited[nex]:
                    continue
                if nex in newFactor.domain:
                    buck[nex].append(newFactor)
                    break
