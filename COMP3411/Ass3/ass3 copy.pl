...
process(thing(Name, _), Ref1, Ref1):-
	thing(Name, Props),
	assert(history(thing(Name, Props))).

% Figure out when to use personal!!!!
...
%process(event(Action, [Actor, Thing]), Ref1, [Action|Ref3]):-	
% call process for child
...
%process(event(Action, [actor(thing(Someone, [isa(person), gender(masculine), number(singular)])), Obje]), Ref1, [Someone| Ref1]):-	
% call process for child
% Add to history
%write("123123"),
%history(event(Action, [actor(thing(Someone, [isa(person), gender(masculine), number(singular)])), Obje])).
